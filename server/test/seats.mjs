...
//
// A seat decides spawn corner and colour, so two players sharing one is a real
// bug (they spawn on top of each other in matching colours). This joins humans
// into a lobby that bots have already filled and checks that each human gets a
...
const table = (room) => [...room.state.players.values()]
  .map((p) => ({ name: p.name, seat: p.seat, bot: p.bot }))
  .sort((a, b) => a.seat - b.seat);

const show = (rows) => rows.map((r) => `seat${r.seat}:${r.name}${r.bot ? '(bot)' : ''}`).join('  ');
...
async function main() {
  ...
  check('lobby topped up to 4', rows.length === 4, `${rows.length}`);
  check('3 of them are bots', rows.filter((r) => r.bot).length === 3);
  ...
  check('still exactly 4 players', rows.length === 4, `${rows.length}`);
  ...
  console.log('   ', show(rows), '\n');
  check('lobby refilled back to 4', rows.length === 4, `${rows.length}`);
